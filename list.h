...
T List<T>::get(uint index) const {
	T aux; // t aux 
	Node<T> *p; //se crea un nuevo node
	p = head; // se le agrega el valor de head a p 

	if (index >= size){ // checa si el index es mayor que el size 
        throw IndexOutOfBounds(); // si lo es muestra esta información
    }
	for (int i = 0; i<index;i++){ //para recorrer nuestro contador de i 
		p = p->next; // se le asigna a p el valor de next 
	
    }
	aux = p->value; // nuestro aux es el p valor
	return aux; // regresa el p valor
}
